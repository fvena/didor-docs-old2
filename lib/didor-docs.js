...
module.exports = async (dir, config) => {
  try {
    ...
    const validateDocsFolder = await pathExists(docsBasePath).then(exists => exists);

    if (!validateDocsFolder) {
      console.log(
        `${chalk.red('Error al generar la documentación:')} No se encuentra el directorio ${dir}`
      );
      return false;
    }
    ...
    server.use(serveStatic(docsBasePath));
