...
module.exports = async configFile => {
  try {
    ...
    const options = {
      ...
      before(app) {
        ...
        app.get('/didor.config.js', (req, res) => {
          ...
          // res.send(`window.$didor = {title:'${config.title}',description:'${config.description}',logo:'${config.logo}',navbar:'${config.navbar}',sidebar:'${config.sidebar}',defaultPath:'${config.defaultPath}',gitRepoLink:'${config.gitRepoLink}', lib:${JSON.stringify(config.lib)}};`);
        });
      },
    };
     ...
     * Configuro webpack
     */
    configWebpack.entry = [path.join(process.cwd(), config.lib.components), path.join(process.cwd(), config.lib.styles)];
