...
const getHiglightCode = (code, lang) => {
  ...
  if (isDemo || isDemoCode) {
    return `<pre v-if="false"></pre><Demo :toggleCode="${isDemoCode}">${encodeURIComponent(code)}<template slot="code"><pre class="line-numbers" data-lang="${language}">${highlight}</pre></template></Demo>`;
  }
